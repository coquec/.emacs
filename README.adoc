...
| `C-_**n**_` .3+| Repite la siguiente orden _**n**_ veces
| `M-_**n**_`
| `C-u _**n**_`
...
[cols="20,80"]
|===
| `C-t` | Transpone los dos caracteres que rodean al cursor y avanza
  el cursor (_hace avanzar_ el carácter que hay antes del cursor y
  deja el cursor tras él). Si está al final de la línea, transpone los
  dos últimos caracteres de la línea sin mover el cursor. Con un
  prefijo `C-_**n**_`, hace avanzar el carácter _**n**_ veces. Con un
  prefijo negativo, lo hace retroceder. Con un prefijo `C-0`,
  transpone el carácter que hay tras el cursor con el de la marca
| `M-t` | Transpone la palabra que hay antes del cursor con la palabra
...
| `C-M-t` | Transpone dos expresiones balanceadas (_hace avanzar_ la
  expresión balanceada que haya encima del cursor)
| `C-x C-t` | Transpone dos líneas (_hace avanzar_ la línea que haya
  encima del cursor)
|===
...
| `C-l` | Pone la línea actual en el centro de la ventana. Pulsar
  varias veces para alternar entre la primera línea, el centro o el
  final de la ventana
